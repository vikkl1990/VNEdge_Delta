...
def test_active_cost_evidence_reprices_every_trade_and_recomputes_pf(
    ...
    assert result["cost_contract"] == "taker_full_14_8"
    assert result["source_route_diagnostics"] == {
        "trades_marked_maker_entry": 0,
        "trades_marked_taker_entry": 2,
...
def test_canonical_cost_contract_overrides_unproven_maker_flag(tmp_path: Path) -> None:
    source = tmp_path / "trades.parquet"
    pd.DataFrame(
        [
            {
    ...
    result = build_active_cost_evidence(source)

    assert result["metrics"]["average_cost_bps"] == pytest.approx(14.8)
    assert result["metrics"]["average_net_bps"] == pytest.approx(5.2)
    assert result["source_route_diagnostics"]["trades_marked_maker_entry"] == 1
