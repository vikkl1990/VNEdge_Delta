...
def build_health_payload(
    ...
    event_states = (
        trigger.get("market_states")
        if isinstance(trigger.get("market_states"), dict)
        else {}
    )
    counterfactual = (
    ...
    feed_delay = (
        recorder.get("feed_delay_us")
        if isinstance(recorder.get("feed_delay_us"), dict)
        else {}
    )
    ...
    for symbol in ("BTCUSD", "ETHUSD"):
        ...
        l2 = (
            ...
            if isinstance(latest.get("l2_confirmation"), dict)
            else {}
        )
        ...
        event_state = (
            event_states.get(symbol)
            if isinstance(event_states.get(symbol), dict)
            else {}
        )
        ...
        markets[symbol] = {
            ...
            "change_24h_pct": None,
            "price_status": (
                "event_tape" if _safe_float(event_state.get("price")) is not None else "not_published"
            ),
            "l2_age_ms": _safe_float(event_state.get("book_age_ms")),
            "l2_fresh": l2.get("status") == "fresh",
            ...
            "active_observation": (
                ...
                if int(counterfactual.get("open") or 0) > 0
                else None
            ),
    ...
    return {
        ...
        "error": None if snapshot_available else "snapshot_unavailable",
        "mode": "RESEARCH",
        "can_trade": False,
        "can_promote": False,
        "order_route": "absent",
        ...
        "feed_delay_corrected_by_channel": (
            recorder.get("feed_delay_corrected_by_channel") or {}
        ),
